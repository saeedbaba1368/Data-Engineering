...
class StreetEasyOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        s3 = S3FileSystem(
            anon=False, key=credentials.access_key, secret=credentials.secret_key
        )
        ...
        with s3.open(s3_path, mode="rb") as s3_file:
            ...
            data = pd.read_csv(
                s3_file, compression="gzip", names=["user_id", "searches"]
            )
            ...
            data["list_of_valid_searches"] = data["valid_searches"].apply(
                list_of_valid_searches
            )
            ...
            unique_valid_searches_df = pd.DataFrame(
                {"searches": list(unique_valid_searches)}
            )

            self.log.info(
                "Total valid searches today are: {}".format(
                    np.sum(data["num_valid_searches"])
                )
            )
            self.log.info(
                "Total users today are: {}".format(
                    np.sum(data["num_valid_searches"] > 0)
                )
            )
        ...
        s3_dest_path = "s3://{}/{}".format(
            self.s3_dest_bucket, rendered_dest_key_no_dashes
        )

        # get a S3 file handle for destination
        s3_dest = S3FileSystem(
            anon=False,
            key=credentials_dest.access_key,
            secret=credentials_dest.secret_key,
        )
        ...
        with s3_dest.open(s3_dest_path, mode="wb") as s3_dest_file:
            ...
            s3_dest_file.write(
                unique_valid_searches_df.to_csv(None, index=False).encode()
            )
        ...
        self.log.info(
            "Rendered Key no dashes {}".format(rendered_dest_df_key_no_dashes)
        )
        s3_dest_df_path = "s3://{}/{}".format(
            self.s3_dest_bucket, rendered_dest_df_key_no_dashes
        )
