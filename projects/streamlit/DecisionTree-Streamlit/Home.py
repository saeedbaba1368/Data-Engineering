...
st.write(
    "choose what type of business you expect to do, choose the location and the state you live in and then check the probability of getting a loan"
)
st.markdown("##")
...
region = st.sidebar.selectbox(
    "Select Zone:", options=df["Region"].unique(), help="segment within a Region"
)
location = st.sidebar.radio(
    "Select  Location:", options=df["Location"].unique(), help="Urban or Rural areas"
)
construction = st.sidebar.radio(
    "select Facility :", options=df["Construction"].unique(), help="goods or services"
)
...
try:
    ...
    df_selection["Region"].replace(
        ["East", "Midwest", "Northeast", "Central"], [1, 2, 3, 4], inplace=True
    )
    df_selection["Construction"].replace(
        ["Frame", "Fire Resist", "Masonry", "Metal Clad"], [1, 2, 3, 4], inplace=True
    )

    X = df_selection.drop(columns=["BusinessType"])
    y = df_selection.drop(
        columns=["Location", "State", "Region", "Investment", "Construction"]
    )
    ...
    with col2:
        ...
        st.markdown("""---""")
        new_user = clf.predict(
            [
                [
                    df_selection["State"].unique(),
                    df_selection["Location"].unique(),
                    df_selection["Region"].unique(),
                    0,
                    df_selection["Construction"].unique(),
                ]
            ]
        )
