...
def main():
    ...
    if file is not None:
        ...
        z_fig = px.line(
            x=z_values1,
            y=prob_values1,
            labels={"x": "Z-score", "y": "Probability Density"},
        )
        z_fig.add_trace(
            go.Scatter(
                x=[z_score],
        ...
        z_fig.add_vline(
            x=0, line=dict(color="red", dash="dash"), name="Mean"
        )  # Add line for mean at Z = 0
        z_fig.update_layout(
            title="", xaxis=dict(showgrid=True), yaxis=dict(showgrid=True)
        )
        ...
        standardized_marks = px.line(
            x=z_values,
            y=prob_values,
            labels={"x": "Z-score", "y": "Probability Density"},
        )
        standardized_marks.add_trace(
            go.Scatter(
                x=[z_score],
                y=[norm.pdf(z_score)],
                mode="markers",
        ...
        standardized_marks.add_annotation(
            x=np.mean(z_values),
            y=max(prob_values),
            text="Mean",
            showarrow=True,
        ...
        standardized_marks.add_annotation(
            x=0, y=max(prob_values), text="Mode", showarrow=True, arrowhead=1, xshift=10
        )
        standardized_marks.add_annotation(
            x=np.std(z_values),
            y=max(prob_values),
        ...
        standardized_marks.add_vline(
            x=np.mean(z_values), line_dash="dash", line_color="red", name="Mean"
        )
        standardized_marks.add_vline(
            x=np.median(z_values), line_dash="dash", line_color="blue", name="Median"
        )
        standardized_marks.add_vline(
            x=0, line_dash="dash", line_color="green", name="Mode"
        )
        standardized_marks.add_vline(
            x=np.std(z_values),
            line_dash="dash",
        ...
        standardized_marks.update_layout(
            title="",
