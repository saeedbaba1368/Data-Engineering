...
state = options_form.selectbox(
    "State", {"Dodoma", "Kilimanjaro", "Dar es Salaam", "Kigoma", "Iringa", "Mwanza"}
)
region = options_form.selectbox("Region", {"East", "Midwest", "Northeast", "Central"})
investment = options_form.number_input("Investment")
construction = options_form.selectbox(
    "Construction", {"Frame", "Metal Clad", "Fire Resist", "Masonry"}
)
businesstype = options_form.selectbox(
    "Business Type",
    {
        "Manufacturing",
        "Apartment",
...
st.dataframe(
    df.tail(st.session_state["number_of_rows"]),
