...
st.success(
    "The main objective is to measure if Number of family dependents and Wives may influence a person to supervise many projects"
)
...
with st.expander("REGRESSION COEFFICIENT EQUESTION OUTPUT"):
    ...
    col2.metric(
        ...
        delta=" for X1 number of Dependant (B1)",
    )
    col3.metric(
        "B2 COEFFICIENT",
        value=f"{coefficients[1]:.4f}",
        delta=" for X2 number of Wives (B2):",
    )
    style_metric_cards(
...
with st.expander("MEASURE OF VARIATIONS"):
    ...
    col3.metric(
        "SUM SQUARED ERROR (SSE):", value=f"{sse:.4f}", delta="Squared(Y-Y_pred)"
    )
    style_metric_cards(
        background_color="#FFFFFF",
        border_left_color="#686664",
