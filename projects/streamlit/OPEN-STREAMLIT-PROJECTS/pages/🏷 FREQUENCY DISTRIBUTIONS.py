...
fig = px.histogram(
    ...
    y=df["UnitPrice"],
    nbins=10,
    labels={"UnitPrice": "UnitPrice", "count": "Frequency"},
    orientation="h",
)
...
fig.add_hline(
    ...
    line_dash="dash",
    line_color="green",
    annotation_text=f"Mean UnitPrice: {mean_unitprice:.2f}",
    annotation_position="bottom right",
)

# Customize marker and line for bars
fig.update_traces(
    marker=dict(color="#51718E", line=dict(color="rgba(33, 150, 243, 1)", width=0.5)),
    showlegend=True,
    name="UnitPrice",
)
...
fig.update_layout(
    ...
    bargap=0.1,
    legend=dict(
        title="Data", orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1
    ),
