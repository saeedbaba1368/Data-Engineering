...
st.info(
    "**Alternative hypothesis: The average Revenue  of Group A and Group B are different.**"
)
...
if sample_size >= 30:
    st.error(
        f" ERROR: T student is for sample size less than 30. unable to solve for **{sample_size}** sample size"
    )
    sys.exit()
...
if abs(t_stat) > critical_value:
    st.success(
        "**✔ REJECT NULL HYPOTHESIS:** The average Revenue  of Group A and Group B are not the same"
    )
else:
    st.success(
        "#### **⚠ FAIL TO REJECT NULL HYPOTHESIS:** The average Revenue of Group A and Group B are the same"
    )

# Plotting the probability density curve
fig.add_trace(
    go.Scatter(
        x=x, y=y, mode="lines", line=dict(color="gray"), name="Probability Density"
    )
)
