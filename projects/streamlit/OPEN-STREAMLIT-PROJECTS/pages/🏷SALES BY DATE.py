...
st.write(
    "Pick a date range from sidebar to view sales trends | the default date is today"
)
...
with st.sidebar:
    ...
    start_date = st.sidebar.date_input(
        "Start Date", date.today() - timedelta(days=365 * 4)
    )

with st.sidebar:
    end_date = st.date_input(label="End Date")
st.error(
    "Business Metrics between[ " + str(start_date) + "] and [" + str(end_date) + "]"
)
...
with b1:
    ...
    with b2:
        ...
        col1.metric(
            label="All Inventory Products ",
            value=df2.Product.count(),
            delta="Number of Items in stock",
        )
        col2.metric(
            label="Sum of Product Price USD:",
            value=f"{df2.TotalPrice.sum():,.0f}",
            delta=df2.TotalPrice.median(),
        )
        ...
        col11.metric(
            ...
            value=f"{ df2.TotalPrice.max():,.0f}",
            delta="High Price",
        )
        col22.metric(
            label="Minimum Price  USD:",
            value=f"{ df2.TotalPrice.min():,.0f}",
            delta="Low Price",
        )
        col33.metric(
            label="Total Price Range  USD:",
            value=f"{ df2.TotalPrice.max()-df2.TotalPrice.min():,.0f}",
...
with a2:
    ...
    bar_chart = (
        ...
        .mark_bar()
        .encode(x="month(Date):O", y="sum(UnitPrice ($)):Q", color="Product:N")
    )
...
with p1:
    ...
    st.subheader(
        "Features by Frequency",
    )
    feature_x = st.selectbox(
        "Select feature for x Qualitative data", df2.select_dtypes("object").columns
    )
    feature_y = st.selectbox(
        "Select feature for y Quantitative Data", df2.select_dtypes("number").columns
    )
...
with p2:
    ...
    source = pd.DataFrame({"Quantity ($)": df2["Quantity"], "Product": df2["Product"]})

    bar_chart = (
        alt.Chart(source)
        .mark_bar()
        .encode(x="sum(Quantity ($)):Q", y=alt.Y("Product:N", sort="-x"))
    )
    st.altair_chart(bar_chart, use_container_width=True, theme=theme_plotly)
