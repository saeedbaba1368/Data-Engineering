...
class ValidSearchStatsOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        with s3.open(s3_path, mode="rb") as s3_file:
            ...
            num_rental_and_sales_searches = np.sum(
                data["type_of_search"] == "rental_and_sale"
            )
            ...
            self.log.info(
                "Total rental searches today are: {}".format(
                    np.sum(data["type_of_search"] == "rental")
                )
            )
            self.log.info(
                "Total sales searches today are: {}".format(
                    np.sum(data["type_of_search"] == "sale")
                )
            )
            self.log.info(
                "Total rental and sales searches today are: {}".format(
                    np.sum(data["type_of_search"] == "rental_and_sale")
                )
            )
            self.log.info(
                "Total none type searches today are: {}".format(
                    np.sum(data["type_of_search"] == "none")
                )
            )
