...
class Heap(): # A modified binary heap
    def __init__(self, arr):
        ...
        self.thread = [i for i in range(len(arr))] # record the index of threads responsible for each thread 
    ...
    def GetMin(self):
        return self.arr[0], self.thread[0] # Return the thread that is the earlist available and its starting time for the new job

    def ChangePriority(self, p):
        self.arr[0] += p # Update the starting time for processing when new job is added
    ...
    def SiftDown(self, i):
        ...
        l = Heap.LeftChild(i)
        if l < len(self.arr):
            if self.arr[l] < self.arr[minIndex]: # Find the thread with smaller starting time (avilable earlier)
                minIndex = l
            if (self.arr[l] == self.arr[minIndex]) & (self.thread[l] < self.thread[minIndex]): # If it happens that two threads are available at the same time, find the thread with smaller index
                minIndex = l
        r = Heap.RightChild(i)
        if r < len(self.arr):
            if self.arr[r] < self.arr[minIndex]: # Find the thread with smaller starting time (avilable earlier)
                minIndex = r
            if (self.arr[r] == self.arr[minIndex]) & (self.thread[r] < self.thread[minIndex]): # If it happens that two threads are available at the same time, find the thread with smaller index
                minIndex = r
        if i != minIndex: # Sift the current thread (which received new job and expected to be available later) down and push the earliest available thread to the root
...
def pq_assign_jobs(n_workers, jobs):
    ...
    for i in range(len(jobs)):
        next_free_time, next_worker = next_free_time_heap.GetMin() # First job can always be processed immediately
        result.append(AssignedJob(next_worker, next_free_time))
        next_free_time_heap.ChangePriority(jobs[i]) # Update thread status for each new coming job, jobs[i] stores the time to process the (i + 1)-th job
    return result
