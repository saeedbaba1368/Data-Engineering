...
class TreeOrders:
    ...
    def inOrderTraversal(self, i, result): # in-order traversal in BST, see tree-order.py
        if i == -1:
            return None
...
def IsBinarySearchTree(tree):
    bst = TreeOrders()
    bst.read(tree)
    result = bst.inOrderTraversal(0, list()) # The result list is a sorted list by the key of all nodes. A key is strictly greater than another key
    return check_result(result) # Apply a lienar comparison algorithm to check if l[current - 1] > l[current]. If so, it violates BST condition
