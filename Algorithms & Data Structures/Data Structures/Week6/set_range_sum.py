...
def find(root, key): # Given root and key, return the node with that key / smallest bigger key and root
    v = root
    last = root
    next = None # If the given key is bigger than all key in the tree, return None
    while v != None: # Terminate the node if v.right or v.left does not exist but v = v.right / v = v.left is called
        ...
        if v.key == key: # If the key is found, done
            break    
        if v.key < key: # If v.key < key, search right subtree of v. Otherwise, search left subtree of v
            v = v.right
        else: 
            v = v.left      
...
def erase(x): # First function to be implemented in assignment: del(i)
    ...
    (middle, right) = split(middle, x + 1) # mid has key less than x + 1. There may be multiple nodes of the same key
    root = merge(left, right)
          
def search(x): # Second function to be implemented in assignment: find(i)
    global root
    (next, root) = find(root, x) # Search for the node given key
    if next != None and next.key == x: # Return true only if the node has the same key as the given key
        return True
