...
class hashed_string: 
    ...
    def hashed_substring(self, start, end): # Return hash values for substring given start and end indices (both inclusively)
        length = end - start + 1 
...
def modified_bs(hashed_p, hashed_t, pattern, substr_text, i, mismatch_count, left, right, k):
    ...
    if right >= left:
        mid = (left + right) // 2
        if substr_text[mid] != pattern[mid]: # If mid characters are not the same for both string, there is a mismatch
            mismatch_count += 1
            
        if hashed_t.hashed_substring(i + left, i + mid - 1) != hashed_p.hashed_substring(left, mid - 1): # Compare the hash values for string in [left, mid - 1]. Do no do recursion if hashes are mismatch
            mismatch_count = modified_bs(hashed_p, hashed_t, pattern, substr_text, i, mismatch_count, left, mid - 1, k)
            
        if hashed_t.hashed_substring(i + mid + 1, i + right) != hashed_p.hashed_substring(mid + 1, right): # Compare the hash values for string in [mid + 1, rigiht]. Do no do recursion if hashes are mismatch
            mismatch_count = modified_bs(hashed_p, hashed_t, pattern, substr_text, i, mismatch_count, mid + 1, right, k)
            
        if mismatch_count > k: # Early stoppling if total mismatch exceeds given bound, k
            return mismatch_count
    return mismatch_count # Return the number of mismatch

def solve(k, text, pattern):
    len_p, len_t, results = len(pattern), len(text), list() # results list store the starting indices of match cases in text with tolerance level of k
    hashed_t, hashed_p = hashed_string(text), hashed_string(pattern) # Hash both strings twice
    for i in range(0, len_t - len_p + 1): # Loop the starting index of text, we compare substrings t[0,...,len(p)], ..., t[len(t) - len(p) + 1, len(t) + 1] with pattern
        substr_text, mismatch_count = text[i:(i + len_p)], 0 # Update substring of text as substr_text and reset the number of mismatch to 0 for each new substring
        left, right = 0, len_p - 1 # Initialize left and right for modified binary search
        mismatch_count = modified_bs(hashed_p, hashed_t, pattern, substr_text, i, mismatch_count, left, right, k)
        if mismatch_count <= k: # If mismatch total is less than or equal to k, append the index
            results.append(i)
    return results
