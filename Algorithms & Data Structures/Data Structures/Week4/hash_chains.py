...
class QueryProcessor: # An object Query Processor
    ...
    def _hash_func(self, s): # Standard implementation of polynomial hashing
        ans = 0
        for c in reversed(s):
            ans = (ans * self._multiplier + ord(c)) % self._prime # ord() maps string to integer using the Unicode
    ...
    def process_query(self, query):
        if query.type == "check": # "check" command query
            if query.ind in self.elems: # Find the chain in hash table with hash key == input number after "check" command, output the words in reversed order 
                self.write_chain(reversed(self.elems[query.ind]))
            else: # If such key does not exist, output an empty string
                self.write_chain('')
        else:
            hashed_string = self._hash_func(query.s) # For other cases, input is a string but not a number. Hash the string first
            ind = -1
            if hashed_string in self.elems: # If the string is found in the hash chain, get the index. Otherwise, index = -1 as default
                if query.s in self.elems[hashed_string]:
                    ind = self.elems[hashed_string].index(query.s)

            if query.type == 'find': # "find" command query
                self.write_search_result(ind != -1) # If the string is found, the index is not -1
                
            elif query.type == 'add': # "add" command query
                if ind == -1: # If the string is not found, check if the hash value is found. If so, append it. If not, create a new hash chain using "hashed_string" as key
                ...
                if ind != -1: # Pop the index from hash chain, it takes at most O(m) using a list
