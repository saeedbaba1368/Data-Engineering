...
def precompute_hashes(T: str, P: int, p: int, x: int) -> list: # Precompute hashing for Rabin-Karp algorithm
    """H: list, y: int"""
    last_index = len(T) - P
    ...
    for i in reversed(range(len(T) - P)):
        H[i] = (x * H[i + 1] + ord(T[i]) - y * ord(T[i + P])) % p # Recurrence relation for hashing (https://en.wikipedia.org/wiki/Rolling_hash)
    return H

def rabin_karp(P: str, T: str) -> list: # Standard implementation of Rabin-Karp algorithm
    p = 10 ** 9 + 19
    x, result = randint(1, p - 1), list()
    p_hash = poly_hash(P, p, x) # Compute hash for pattern
    H = precompute_hashes(T, len(P), p, x) # Precompute all hashes for text (T) with length equal to pattern (P)
    for i in range(len(T) - len(P) + 1): # Compare hashes, if same hashes, then compare string and append the matching results (starting index)
        if p_hash != H[i]:
            continue
        if T[i:(i + len(P))] == P:
            result.append(i)
