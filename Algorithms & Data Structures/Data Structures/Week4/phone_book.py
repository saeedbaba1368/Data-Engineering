...
class Query:
    def __init__(self, query):
        self.type = query[0] # For each query, self.type includes 3 operations: "add", "find" and "del"
        self.number = int(query[1]) # Phone number in each query
        self.hash_number = hash_num(self.number) # Hash the phone number
        if self.type == 'add':
            self.name = query[2] # For "add" operation, the query has 3 elements and the last element in name
...
def process_queries_fast(queries):
    ...
    for cur_query in queries:
        ...
        if cur_query.type == 'add': # Add a new contact to hash table
            if cur_query.hash_number in contacts: # If hash number is already in the dictionary
                for contact in contacts.get(cur_query.hash_number): # If query phone number appears in the list, update name and break the loop
            ...
            if cur_query.hash_number in contacts: # If the hash number is found in the contact
                for i in range(len(contacts.get(cur_query.hash_number))):
                    if contacts[cur_query.hash_number][i].number == cur_query.number: # If the phone number is matched, pop it and break the loop
            ...
            if cur_query.hash_number in contacts: # If the hash number is found in the contact
                for contact in contacts[cur_query.hash_number]:
                    if contact.number == cur_query.number: # If the phone number is matched, record the name and break the loop
