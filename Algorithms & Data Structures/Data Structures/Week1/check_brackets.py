...
def find_mismatch(text):
    ...
    if len(text) == 1: # If the text only contains a character, the mismatch position must be at 1
    ...
    for i, char in enumerate(text): # A loop of each character in the given text
        ...
        if char in ")]}": # Otherwise, if char is closing bracket, do the following:
            if len(opening_brackets_stack) == 0: # If the queue is empty, it means that opening brackets are missing in the queue. So, return the current index as mismatch position
                ...
                (top[0] == '{' and char != '}'): # Check if the opening bracket match the closing bracket. If not, return the current position
                    return i + 1
         # Ignore the cases that char is neither opening nor closing brackets
    if len(opening_brackets_stack) == 0: # If all opening brackets are popped out, all brackets are matched
        return('Success')
