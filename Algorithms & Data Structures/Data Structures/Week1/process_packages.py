...
class Buffer:
    ...
    def process(self, request):
        if len(self.finish_time) == 0: # If finish_time is empty queue, then it is ready to accept any requests
            est_finish_time = request.arrived_at + request.time_to_process # Estimated finish time = Arrival time + Time to process the request
            self.finish_time.append(est_finish_time)
            return Response(False, request.arrived_at) # So, the request will be processed at arrival time
        else: # Otherwise, there are some requests not processed in the queue
            act_finish_time = self.finish_time[0]
            if act_finish_time <= request.arrived_at: # If the first request can be finished before the arrival of the current request, pop it
                self.finish_time.popleft()
            if len(self.finish_time) ==  0: # If it is an empty list after the first request is popped, repeat the previous process for empty queue
                ...
                if prev_finish_time > request.arrived_at: # If the last request cannot be finished before the current request arrives and we have space to accomodate an extra request
                    if self.size > len(self.finish_time):
                        self.finish_time.append(prev_finish_time + request.time_to_process) # then add to the buffer list and it will be processed when the last request is finished
                    ...
                    if self.size > len(self.finish_time):
                        self.finish_time.append(request.arrived_at + request.time_to_process) # then add to the buffer list and it will be processed as soon as it arrives
                        return Response(False, request.arrived_at)
            return Response(True, request.arrived_at) # Otherwise, it must be the case that buffer size is fulled when an extra request comes, and it will be dropped
...
def main():
    ...
    for _ in range(n_requests):
        arrived_at, time_to_process = map(int, input().split()) # Read arrival time and time to process for each request and store them as a Request object in requests list.
