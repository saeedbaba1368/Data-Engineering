...
class NegWCycle():
    ...
    def __init__(self, adj, cost, s, distance, reachable, shortest):
        self.s = s
        self.dist = distance.copy() # This is the distance array in BFS for negative cycle, not for Bellman-Ford algorithm in detecting negative cycle
        self.adj = adj
        self.cost = cost
        self.q = queue.Queue()
            ...
            for u in range(n): # Run all edges
                if len(self.adj[u]) != 0:
                    for i in range(len(self.adj[u])):
                        v = self.adj[u][i]
                        if distance[v] > distance[u] + self.cost[u][i]: # Edge relaxation
                            distance[v] = distance[u] + self.cost[u][i] # update the distance in edge relaxation
                            reachable[v] = 1 # update the reachability
                            if j == n - 1: # If there is edge relaxation in the |V|-th iteration, it imples a negative cycle
    ...
    def search_cycle(self):
        ...
        while not self.q.empty():
            u = self.q.get() # dequeue an element, if it is the first element in the queue, let distance to itself is set to 0, and update the boolean
            if first == True:
                self.dist[u] = 0
                first = False
            shortest[u] = 0 # Every such node dequeued is a part of negative weight cycle; hence it does not belong to the shortest path
            for v in self.adj[u]: # search the neighborhood of u
                if self.dist[v] == inf: # if it's not visited before, enqueue it and d(v, s) = d(u, s) + 1 because each edge has the same weighting
                    self.q.put(v) # enqueue all its unvisited neighborhood; update distance
