...
def explore(v, adj, visit): # a depth first search
    ...
    for w in adj[v]:
        if visit[w] < 2: # if visit of neighborhood is at most 1, we continue the depth first search. Otherwise, there is a cycle (visit[w] >= 2) and the recursion is infinite
            explore(w, adj, visit)
    return visit
...
def acyclic(adj):
    for i in range(len(adj)):
        bool_cycle = reach(adj, i, i) # If there is a cycle which consists of i-th node, it must be reachable to itself
        if bool_cycle == 1:
