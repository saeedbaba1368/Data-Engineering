...
def explore(v, visit, adj, pre, post, clock, stack): 
    ...
    stack.append(v) # every visited node is appended to a LIFO queue in ascending order of postvisit (largest postvisit order at the end of queue)
...
def toposort(adj):
    ...
    n = len(stack)
    for i in range(n): # same as reversed(stack) because topological sort put node in descending order of postvisit
