...
def download_object(version):
    ...
    with open(f'backfill\\{file_directory}\\{date}-{version_id}-{filename}', 'wb') as fout:
        try:
...
def download_parallel_multiprocessing(filename = ""):
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        future_to_key = {executor.submit(download_object, version): version for version in versions}
...
if __name__ == "__main__":
    ...
    for tab in REQUIRED_TABLES:
        ...
        for fname in os.listdir(path_to_local):
            ...
            arguments.append(
                (full_path_to_local, BUCKET, path_to_s3)
            )