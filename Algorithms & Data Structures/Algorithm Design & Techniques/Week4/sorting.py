...
def partition3(a, l, r): 
    ...
    while i <= greater_than:
        if a[i] < x: # Swap and make item smaller than the pivot on left hand side
            ...
            a[i], a[greater_than] = a[greater_than], a[i] # Swap the items at the end and i-th position
            greater_than -= 1  # Since item at the end must be greater than pivot, decrease it by 1
        else: # Same value then shift a unit to the right for next comparison
            i += 1
    return less_than, greater_than # It gives the smallest index and the largest index of same value in the array having value of a[l]
