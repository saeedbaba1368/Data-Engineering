...
def modified_binary_search_inf(array, value, l, r): # Time complexity: O(logn)
    ...
    if value == array[m]: # If there is an exact match, do backward linear search
        while value == array[m]:
            m -= 1
            if m < 0: # Case if given point is the minimum, then no largest ending points => return -1
                return -1
        return m # Otherwise, return such point
    if l == r:
        if array[l] < value: # If there is no exact match, check if the ending point found is smaller than the given point. If so, return it.
            return l
        return l - 1 # If not, it's possible that 1) array[m] < value < array[m + 1] or 2) value < array[m]. First case => set l := m+1 and r := r. When l == r, it's possible lower bound on a unit left. If original index is already 0, it will return -1.
    elif value < array[m]:
...
def modified_binary_search_sup(array, value, l, r): # Time complexity: O(logn)
    ...
    if value == array[m]: # If there is an exact match, do forward linear search
        while value == array[m]:
            m += 1
            if m > len(array) - 1: # Case if given point is the maximum, then no smallest starting points => return -1
                return -1
        return m # Otherwise, return such point
    if l == r:
        if array[l] > value: # If there is no exact match, check if the starting point found is greater than the given point. If so, return it.
            return l
        return -1 # If not, return not found. As value > array[m] will push l to m + 1 when array[m] < value < array[m + 1], such case must be value > array[len(array) - 1] 
    elif value > array[m]:
...
def fast_count_segments(starts, ends, points):
    ...
    for p in points:
        p_sup = modified_binary_search_sup(sort_starts, p, index_begin, index_stop) # For a point p, we find the smallest starting point above p. 
        p_inf = modified_binary_search_inf(sort_ends, p, index_begin, index_stop)  # For a point p, we also find the largest ending point below p.
        num_segment_above, num_segment_below = (len(starts) - p_sup)*(p_sup > -1), (p_inf + 1)*(p_inf > -1) # The number of segment above p is total number of starting points - the index of the smallest starting point. The number of segment below p is the index of the largest ending point + 1 (python starts at index 0)
        num = len(starts) - (num_segment_above + num_segment_below) # Boolean variables are introduced in case if no segment above/below a certain point, set to 0. So, total number of segments covering p = total number of segments - total number of excluded segments.
        cnt.append(num)
    return cnt
