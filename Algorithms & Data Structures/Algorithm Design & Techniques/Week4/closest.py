...
def merge(subarray_1, subarray_2, key):
    ...
    subarray_1 = deque(subarray_1) # Use deque as the grader has a strict time limit. list.pop(0) for every loop is more expensive than deque.popleft(), which costs constant per iteration.
    subarray_2 = deque(subarray_2) # Converting data type costs linear w.r.t. subarray length
    array_sorted = list()
...
def checking(point_strip, guess_dist):
    ...
    for i in range(0, n):
        ...
        point_compare = point_sorted_y[i + 1:min(i + 7, n)] # We need to check at most 6 points with pairwise distances at least guess_dist: https://en.wikipedia.org/wiki/Closest_pair_of_points_problem
...
def split_halves(point):
    ...
    if n <= 3: # Base case when having 3 or less points. Iterate through all combinations and return the minimum distance between pairs
...
def minimum_distance(x, y):
    ...
    min_dist = split_halves(point_sorted_x) # Split all points into 2 symmetric to x = c
    return min_dist
