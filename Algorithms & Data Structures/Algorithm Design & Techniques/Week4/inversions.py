...
def get_number_of_inversions(a, b, left, right): # Array b is given in the problem but I don't use it.
    ...
    number_of_inversions += get_number_of_inversions(a, b, left, ave) # Accumulate the number of inversions
    number_of_inversions += get_number_of_inversions(a, b, ave, right) # Accumulate the number of inversions
    # For more than one element in the array
    left_list, right_list = a[left:ave].copy(), a[ave:right].copy()
    d = []
    while (left_list != []) & (right_list != []):
        left_first, right_first = left_list[0], right_list[0] # Standard merge sort procedure
        if left_first <= right_first: 
            ...
            del right_list[0]
            number_of_inversions += len(left_list) # If we append something on the right, it implies an inversion. 
