...
def dp_optimal_sequence(n): 
    MinOpt, rlist = [0] * n, [] # MinOpt: Record minimum number of operations for each integer upto n, rlist: Record a sequence of number leading to n
    MinOpt[0] = 0  # No operation is needed for 1, so MinOpt[1 - 1] = 0
    for i in range(n): # Let i denote the index of MinOpt. Minopt[0] is the minimum number of operations for number 1. Similarly, Minopt[n - 1] is associated with number n
        ...
        for next_pos in next_list:
            if next_pos < n: # It's possible one of the calculation produce a number bigger than n. Just ignore it.
                if MinOpt[next_pos] == 0: # If the next position hasn't been filled, just filled with (the minimum operations of k) plus 1
                    MinOpt[next_pos] = MinOpt[i] + 1
                elif MinOpt[i] + 1 < MinOpt[next_pos]: # If the next posistion has been filled in previous DP process, compare if the current move requires fewer operations
                    MinOpt[next_pos] = MinOpt[i] + 1
    rlist.append(n) # Backtracking technique
    count = MinOpt[-1] - 1 # It should take at most MinOpt[n - 1] steps (calculated from dynamic programming) to reduce n to 1. Minus 1 because we start from 0.
    while count >= 0:
        last_num = rlist[-1] # Start from the end, integer n
        memo = [] # Contain three possible operations to reach n
        memo.append(last_num - 2) # First, index[k] = index[k'] - 1 = k' - 1 - 1 = k' - 2
        if last_num % 2 == 0: # If k' is divisible by 2, then index[k] = (index[k'] + 1) / 2 - 1 = k'/2 - 1
            memo.append(last_num // 2 - 1)
        if last_num % 3 == 0: # If k' is divisible by 3, then index[k] = (index[k'] + 1) / 3 - 1 = k'/3 - 1
            memo.append(last_num // 3 - 1)
        min_index = 0
        for i in range(len(memo)): # Consider all possible operations to get k' from the primitive calculator, and find out the operation which is the minimum in total
