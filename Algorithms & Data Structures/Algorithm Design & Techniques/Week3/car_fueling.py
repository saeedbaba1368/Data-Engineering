...
def compute_min_refills(distance, tank, stops):
    ...
    num_refills, current_refill = 0, 0 # Initially, number of refills is 0; current refill status is 0
    
    while current_refill <= n: # When current_refill <= n, we haven't arrived the destination
        ...
        if current_refill == last_refill: # This occurs if the above while loop is not executed i.e. cannot reach destination with full tank
            return -1
        if current_refill <= n: # After the above while loop is finished, the farthest stop is reached for refilling, so the number of refills + 1
            num_refills += 1
