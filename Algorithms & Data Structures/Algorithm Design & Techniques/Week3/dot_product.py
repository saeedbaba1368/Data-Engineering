...
def search_max(a): # Linear search for the index corresponding to the maximum value in a list
    index = 0
...
def max_dot_product(a, b):
    ...
    for i in range(n): # For loop costs O(n) and search_max() costs 2*O(n). In total, running complexity is O(n^2).
