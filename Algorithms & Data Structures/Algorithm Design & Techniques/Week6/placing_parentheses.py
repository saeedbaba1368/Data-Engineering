...
def get_maximum_value(dataset):
    ...
    m, M = [n * [0] for i in range(n)], [n * [0] for i in range(n)] # Let m, M be the minimum and the maximum value of subexpressions, resp
    
    for i in range(len(dataset)): # Let i denote the index of the input array, with digits and operators mixed together
        if i % 2 == 0: # If i mod 2 == 0: dataset[i] contains a digit
            pos = i // 2
            m[pos][pos], M[pos][pos] = int(dataset[i]), int(dataset[i]) # When the subexpression is a digit, 
    ...
    for s in range(n - 1): # Double for-loops to update the upper diagonal of m and M
        for i in range(n - s - 1):
            j = i + s + 1 # because j - i = s + 1 which ensures the upper diagonal elements of m and M are updated
