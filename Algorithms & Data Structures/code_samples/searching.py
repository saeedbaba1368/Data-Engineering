...
class Search:
    ...
    def fibonacci_search(self, target, sequence):
        ...
        while (fibM > 1):
            ...
            if (sequence[i] < target):
                ...
                return i
        if(fibM_minus_1 and index < (len(sequence)-1) and sequence[index+1] == target):
            return index+1;
        return -1
    ...
    def interpolation_search(self, target, sequence):
        ...
        while low <= high and target >= sequence[low] and target <= sequence[high]:
            index = low + int(((float(high - low) / ( sequence[high] - sequence[low])) * ( target - sequence[low])))
...
class SortedCollection(Search):
    ...
    def search(self,target,algorithm = "binary_search"):
        ...
        if algorithm not in methods:
            raise Exception("Not an acceptable algorithm, select one of the following: {}".format(methods))
