...
def MarkPivotElementUsed(pivot_element, used_rows, used_columns): # see energy_values.py
...
def Evaluate(n, m, c, x): # evaluate the objective function of the linear program given solution and coefficients
    opt_val = 0
...
def CheckInequality(n, m, A, b, solution): # check if the solution indeed satisfies all the constraints given in the linear program
    status = True
...
def CheckInfinity(n, m, equations): # check if the solution if unbounded
    ...
    for j in equations.b_: # If the solution is bounded, one element in solution will goes to infinity (i.e. max_cap)
        if j == max_cap:
            return True
    return False

def solve_diet_problem(n, m, A, b, c):  
    A, b = AppendEquations(n, m, A, b) # Append constraints to Ax <= b to avoid unwanted solution which also yields the maximum
    A, b = ChangeDataType(A, b) # Change data type for A and b
    collection = CreateSubset(n, m, A, b) # Consider different combinations of m inequalities out of all and solve for the optimal solution one by one
    solutions, opt_vals, store_equations = [], [], []
    
    for equations in collection:
        solution = SolveEquation(equations) # Solve tight equations for those m inequalities using Gaussian elimination
        if solution == False: # if there is no solution, proceed to the next set of tight inequalities
            continue
            
        status = CheckInequality(n, m, A, b, solution) # If there is solution, check if the solution is feasible. Ignore non feasible solution
        if status == True: # If the solution lies in constraints, evaluate the optimal value and store the solutions, corresponding inequalities and the optimal values to three lists
    ...
    for index, value in enumerate(opt_vals):
        if value > max_value:
            max_value = value
            max_index = index
    zero_solution = [0] * m # we also consider a trivial solution, that is no consumption of any diets
    status = CheckInequality(n, m, A, b, zero_solution) # check if zero solution is feasible 
...
n, m = list(map(int, stdin.readline().split())) # n = number of equations, m = number of state variables
A = []
for i in range(n): # from row 1 to row n in given input, they are coefficients for the constraint
    A += [list(map(int, stdin.readline().split()))]
b = list(map(int, stdin.readline().split())) # the second last line is a constant vector for the constraint s.t. Ax <= b
c = list(map(int, stdin.readline().split())) # the last line is a vector for the linear problem such that max c*x
